AFRAME.registerComponent("addmarker",{
    init: async function(){
      ...
      if (!dish.unavailable_days.includes(days[todaysDay])) {
            ...
            var ingredientes = document.createElement("a-plane")
            ingredientes.setAttribute("id", `plane-${dish.id}`)
            ingredientes.setAttribute("position", {x: 0, y: 0, z: 0})
            ingredientes.setAttribute("rotation", {x: -90, y: 0, z: 0} )
            ingredientes.setAttribute("width", 1.7)
            ingredientes.setAttribute("height", 1.5)
            ingredientes.setAttribute("visible", false)

            marker.appendChild(ingredientes)
            ...
            ingredientes.appendChild(ingText)
