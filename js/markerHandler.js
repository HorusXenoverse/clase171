...
AFRAME.registerComponent("markerhandler",{
    ...
    alertWelcome: function(){
        ...
        swal({
            title: "Bienvenidos a el restaurant",
            icon: inconSave,
            content: {
                element: "input",
                attributes: {
    ...
    if (dish.unavailable_days.includes(days[todaysDay])) {
      swal({
        ...
        order_button.addEventListener("click", function(){
            var numberTable
            tableNumber <= 9 ?(numberTable = `t0${tableNumber}`) : `t${tableNumber}`
            this.updateDataBase(numberTable, dish)
            swal({
                icon: "https://i.imgur.com/4NZ6uLY.jpg", title: "Gracias por tu pedido", text: "recibirás tu orden pronto"
            })
        })

        raiting_button.addEventListener("click", function(){
            swal({
                icon: "warning", title: "Calificar platillo", text: "Gracias por calificar"
            })
        }) 
        }
      }
